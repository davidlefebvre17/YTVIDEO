...
async function main() {
  console.log('=== Narrative v5c (no priming examples in guidance) — 3 beats ===\n');
  ...
  const outputDir = path.join(episodeDir, 'images-narrative-v5c');
  fs.mkdirSync(outputDir, { recursive: true });
  fs.writeFileSync(
    path.join(outputDir, '_v5c.json'),
    JSON.stringify({
      date: DATE, segId: SEG_ID, segmentTitle: section.title, version: 'v5c',
