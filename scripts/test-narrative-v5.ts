...
- For a named-company: iconic architecture, signature products, characteristic environments, gestures of its activity
- For a named-institution: distinctive buildings, ritual settings, attire, signature objects
- For a named-person: physical traits, characteristic gestures, recurring contexts (Oval Office, podium, etc.)
- For an unnamed-actor: gestures, clothing, environment that reveal the role
- For a financial-symbol: physical embodiment of the asset (matter, commodity, banknotes, instruments)
- For an economic-indicator: physical instruments (gauges, dials, levers, scales)
- For a physical-object: precise sensory detail
- For a document-screen: layouts, charts, tables, columns, signature visual elements
...
async function main() {
  console.log('=== Narrative v5 (visual representations + ticker cleanup) — 3 beats ===\n');
  ...
  const outputDir = path.join(episodeDir, 'images-narrative-v5');
  fs.mkdirSync(outputDir, { recursive: true });
  fs.writeFileSync(
    path.join(outputDir, '_v5.json'),
    JSON.stringify({
