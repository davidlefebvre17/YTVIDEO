...
const PREV_VIDEO = path.join(ROOT, "data", "owl-paris-V8-walk-to-left-table.mp4");
const LAST_FRAME = path.join(ROOT, "data", "owl-paris-V8-last-frame.png");
const END_FRAME = path.join(ROOT, "data", "owl-paris-K1-clean.png");
const OUTPUT = path.join(ROOT, "data", "owl-paris-V9-sit-down.mp4");

// eslint-disable-next-line @typescript-eslint/no-require-imports
const FFMPEG = (require("ffmpeg-static") as string);

const VIDEO_PROMPT = `The owl walks the last few steps from his standing position next to the coffee table over to the lounge armchair, then sits down in the chair and ends in a relaxed reclined pose with both feet propped up on the marble coffee table, ankles crossed. He keeps speaking continuously throughout the ENTIRE 5-second clip — his beak opens and closes regularly and rhythmically without ever stopping, from the very first frame to the very last frame. Camera completely static, only the owl moves, the room stays perfectly still.`;
...
async function main() {
  ...
  const startFile = new File([fs.readFileSync(LAST_FRAME)], "owl-V8-last.png", {
    type: "image/png",
  });
