...
type StagingMode = 'co-located' | 'confronted' | 'anchored' | 'solo';
...
[
  {
    ...
    "staging": {
      "mode": "<one of 4>",
      "compositionHint": "<10-30 words describing the unified scene where ALL subjects coexist>"
    }
  }
]`;
...
async function main() {
  console.log('=== Narrative v5d (no priming examples in guidance) — 3 beats ===\n');
  ...
  // PASS 2
  console.log('\n→ PASS 2: narrative rich prompts (parallel, with staging)...');
  for (const sb of subjectsByBeat) {
    console.log(`  [${sb.id}] staging.mode=${sb.staging?.mode}  hint="${sb.staging?.compositionHint}"`);
  }
  const t2 = Date.now();
  const beatDataById = new Map(subjectsByBeat.map((s) => [s.id, { subjects: s.subjects, staging: s.staging }]));
  const results = await Promise.all(
    beats.map(async (beat) => {
      const data = beatDataById.get(beat.id);
      const subjects = data?.subjects ?? [];
      const staging = data?.staging ?? { mode: 'solo' as StagingMode, compositionHint: 'single subject' };
      try {
        const output = await runPass2OneBeat(beat, subjects, staging, section);
        return { beat, subjects, staging, output, error: null as null | string };
      } catch (e) {
        return { beat, subjects, staging, output: null, error: (e as Error).message.slice(0, 120) };
      }
    }),
  );
  console.log(`  ✓ ${results.filter((r) => r.output).length}/${results.length} prompts in ${((Date.now() - t2) / 1000).toFixed(1)}s\n`);

  for (const { beat, subjects, staging, output, error } of results) {
  ...
  const outputDir = path.join(episodeDir, 'images-narrative-v5d');
  fs.mkdirSync(outputDir, { recursive: true });
  fs.writeFileSync(
    path.join(outputDir, '_v5d.json'),
    JSON.stringify({
      date: DATE, segId: SEG_ID, segmentTitle: section.title, version: 'v5d',
      results: results.map(({ beat, subjects, staging, output, error }) => ({
        beatId: beat.id, narration: beat.narrationChunk, subjects, staging, narrative: output, error,
      })),
    }, null, 2),
    'utf-8',
  );
