...
const editorial: EditorialPlan = {
  ...
  moodMarche: flagged.sentiment?.value !== undefined && flagged.sentiment.value < 30 ? "risk-off"
    : flagged.sentiment?.value !== undefined && flagged.sentiment.value > 70 ? "risk-on"
    : "incertain",
...
// ── Simulate briefing assembly (no Haiku, just top 15) ──
console.log("\n── Simulated Briefing (top 15, no Haiku) ──");
...
const metaMap = new Map(index.chunks.map((c: any) => [c.id, c]));
...
for (const c of top15) {
  const filePath = path.join(CHUNKS_DIR, `${c.id}.md`);
  if (!fs.existsSync(filePath)) continue;
  const raw = fs.readFileSync(filePath, "utf-8");
  const content = raw.slice(raw.indexOf("---", 3) + 3).trim();
  const chunkTokens = Math.ceil(content.split(/\s+/).length * 1.3);
  if (tokens + chunkTokens > TOKEN_BUDGET && tokens > 0) break;
  const meta = metaMap.get(c.id);
  briefing += `### ${meta?.title ?? c.id}\n${content}\n\n`;
  tokens += chunkTokens;
}

console.log(`Briefing: ${tokens} tokens, ${briefing.length} chars`);
console.log(`Chunks included: ${top15.map(c => c.id).join(", ")}`);

// Show first 500 chars of briefing
console.log("\n── Briefing preview (first 800 chars) ──");
console.log(briefing.slice(0, 800) + "\n...");

console.log("\n✓ Knowledge RAG live test complete");
