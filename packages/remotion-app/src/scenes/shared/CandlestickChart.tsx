 ...
 * After draw: magnifier lens zooms into last 15 candles (nested SVG, no pixelation).
 */
...
export const CandlestickChart: React.FC<CandlestickChartProps> = ({
  ...
  // Filter out candles with null/0 prices (unfinished trading day)
  const cleanCandles = useMemo(
    () => candles.filter(c => c.c != null && c.o != null && c.h != null && c.l != null && c.c !== 0),
    [candles],
  );
  ...
  const { minP, maxP, range } = useMemo(() => {
    if (cleanCandles.length === 0) return { minP: 0, maxP: 1, range: 1 };
    const lo = Math.min(...cleanCandles.map(c => c.l));
    const hi = Math.max(...cleanCandles.map(c => c.h));
  ...
  const smaData = useMemo(() => {
    if (!showSMA) return [];
    return smaWindows.map(w => computeSMA(cleanCandles, w));
  }, [cleanCandles, showSMA, smaWindows]);

  const rsiValues = useMemo(() => showRSI ? computeRSI(cleanCandles, 14) : [], [cleanCandles, showRSI]);

  const gridCount = BRAND.chart.gridLines;
  const gridLines = useMemo(() =>
    Array.from({ length: gridCount + 1 }, (_, i) => {
      const ratio = i / gridCount;
      return { y: PAD_V + ratio * (priceH - 2 * PAD_V), price: maxP - ratio * range };
  ...
  // (magnifier removed — zoom is handled by DataOverlay with two chart instances)
        ...
        {gridLines.map(({ y, price }, i) => (
          <g key={`g${i}`}>
            ...
            <text x={PAD_H - 8} y={y + 4} textAnchor="end" fontSize={10}
              ...
              {price >= 1000 ? price.toLocaleString("fr-FR", { maximumFractionDigits: 0 }) : price.toFixed(2)}
            </text>
          </g>
        ))}
        ...
        {showRSI && rsiH > 0 && (() => {
          ...
          return (
            <g opacity={rsiOpacity}>
              ...
              {path && <path d={path} fill="none" stroke={accentColor} strokeWidth={1.5} />}
